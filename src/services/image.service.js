...
const generateSummaryImage = async () => {
  try {
    ...
    // 2. Create an SVG image using the fetched data
    // SVG is a text-based format, making it easy to construct an image with dynamic text.
    const svg = `
    ...
    // We log the error but don't throw it, so a failure here
    // doesn't cause the entire /refresh endpoint to fail.
  }
};
